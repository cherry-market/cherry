...
import { MAIN_TABS, MAIN_TAB_LABELS } from '@/shared/constants/navigation';
    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 flex justify-center pointer-events-none">
      <div className="w-full max-w-[430px] bg-white/90 backdrop-blur-xl border-t border-gray-100 pb-[env(safe-area-inset-bottom)] shadow-[0_-4px_20px_rgba(0,0,0,0.02)] pointer-events-auto border-x border-gray-100">
        <div className="flex justify-between items-center px-6 h-16">
          <NavButton
            icon={Home}
            label={MAIN_TAB_LABELS.HOME}
            isActive={activeTab === MAIN_TABS.HOME}
            onClick={() => onTabChange(MAIN_TABS.HOME)}
          />
          <NavButton
            icon={MessageCircle}
            label={MAIN_TAB_LABELS.CHAT}
            isActive={activeTab === MAIN_TABS.CHAT}
            onClick={() => onTabChange(MAIN_TABS.CHAT)}
          />
          <NavButton
            icon={Cherry}
            label={MAIN_TAB_LABELS.LIKES}
            isActive={activeTab === MAIN_TABS.LIKES}
            onClick={() => onTabChange(MAIN_TABS.LIKES)}
          />
          <NavButton
            icon={User}
            label={MAIN_TAB_LABELS.MY}
            isActive={activeTab === MAIN_TABS.MY}
            onClick={() => onTabChange(MAIN_TABS.MY)}
          />
        </div>
      </div>
    </div>
