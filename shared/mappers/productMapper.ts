...
export class ProductMapper {
  static toFrontend(backend: ProductSummary): Product {
    return {
      ...
      title: backend.title,
      price: backend.price,
      image: backend.thumbnailUrl || '/cherry_logo_profile.svg',
      status: backend.status,
      tradeType: backend.tradeType === 'BOTH' ? 'ALL' : backend.tradeType,
      images: backend.thumbnailUrl ? [backend.thumbnailUrl] : ['/cherry_logo_profile.svg'],
      description: '',
  ...
  static toFrontendFromDetail(backend: ProductDetail): Product {
    return {
      ...
      title: backend.title,
      price: backend.price,
      image: backend.imageUrls?.[0] || '/cherry_logo_profile.svg',
      status: backend.status,
      tradeType: backend.tradeType === 'BOTH' ? 'ALL' : backend.tradeType,
      images: backend.imageUrls?.length ? backend.imageUrls : ['/cherry_logo_profile.svg'],
